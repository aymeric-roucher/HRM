...
def get_device():
    ...
    if torch.backends.mps.is_available() and torch.backends.mps.is_built():
        ...
        warnings.warn("No GPU found, using CPU, this will be slow!")
        return "cpu"
...
def train_batch(
    ...
    if len(metrics):
        ...
        if rank == 0:
            # Convert metric tensors to Python floats
            reduced_metrics = {
                k: float(metric_values[i].item()) for i, k in enumerate(metric_keys)
            }

            # Postprocess
            count = max(float(reduced_metrics["count"]), 1.0)  # Avoid NaNs
            reduced_metrics = {
                f"train/{k}": (
                    v / (float(global_batch_size) if k.endswith("loss") else count)
                )
